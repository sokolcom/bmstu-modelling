...
def main():
    ...
    while True:
        ...
        while True:
            ...
            # else:
            #      config.F0 = -5
            # config.F0 = ti ** 3 + 2 * ti ** 2 - 6 * ti - 5
            # if config.F0 > 67:
                # config.F0 = 67
            # config.F0 = math.pow(math.e, -ti)
            print(f"F0 = {config.F0}")
    ...
    for i in res:
        if (step1 % 2 == 0):
            # plt.plot(x, i[:-1], label=f"t_n = {te[step1]} sec")
            idx += 1
        step1 += 1
        if idx == 5:
            break

    # plt.plot(x, res[-1][:-1], label=f"t_n = {te[-1]} sec")
    # plt.xlabel("x, cm")
    # plt.ylabel("T, K")
    # plt.grid()
    # plt.legend()
    ...
    while (step2 < l / 3):
        ...
        plt.plot(te, point[:-1], label=f"x_n = {x[int(step2 / h)]}cm")
        step2 += 0.1
        i += 1
        if i == 5:
            break
